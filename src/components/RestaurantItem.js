...
export default function RestaurantItem(props) {
  return (
   <>
    <ScrollView>
      {props.restaurantData.map((rest, index) => (
        <TouchableOpacity key={index} activeOpacity={1} style={{marginBottom: 30}} onPress={() => console.log(rest)}>
          <View style={{marginTop: 10, backgroundColor: 'white', padding: 15}}>
            <RestaurantImage image={rest.image_url} />
            <RestaurantInfo name={rest.name} rating={rest.rating}  />
          </View>
        </TouchableOpacity>
      ))}
    </ScrollView>
